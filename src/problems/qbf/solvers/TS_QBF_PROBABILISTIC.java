 ...
 */
public class TS_QBF_PROBABILISTIC extends AbstractTS<Integer> {
     ...
     */
    public TS_QBF_PROBABILISTIC(Integer tenure, Integer time, String filename, Double temperature) throws IOException {
    ...
    public Solution<Integer> neighborhoodMove() {
        ...
        for (Integer candIn : CL) {
            ...
            if (rand < probability && (!TL.contains(candIn) || sol.cost + deltaCost < bestSol.cost)) {
        ...
        for (Integer candOut : sol) {
            ...
            Double probability = Math.exp(-deltaCost / temperature);
            Double rand = Math.random();
            if (rand < probability && (!TL.contains(candOut) || sol.cost + deltaCost < bestSol.cost)) {
                if (deltaCost < minDeltaCost) {
                    minDeltaCost = deltaCost;
                    bestCandIn = null;
                    bestCandOut = candOut;
        ...
        for (Integer candIn : CL) {
            for (Integer candOut : sol) {
                ...
                double probability = Math.exp(-deltaCost / temperature); // Boltzmann probability
                double rand = Math.random();
                if (rand < probability && ((!TL.contains(candIn) && !TL.contains(candOut)) || sol.cost + deltaCost < bestSol.cost)) {
                    if (deltaCost < minDeltaCost) {
                        minDeltaCost = deltaCost;
                        bestCandIn = candIn;
                        bestCandOut = candOut;
